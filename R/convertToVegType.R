...
#browser()
  ...
  zeroCutoff = 0.05
  #message("AAAA ", DT$leading)
  #message("BBBB ", .sumRelBs(deciSp, DT))
  if (any(DT$leading %in% pine) &&
    ...
    "Pine"
  } else if (any(DT$leading %in% c("Pice_mar")) &&
    ...
    "Mixed"
              ...
              (.sumRelBs("Lari_lar", DT) >= zeroCutoff ||
               .sumRelBs(c("Popu_tre", "Popu_bal", "Abie_bal", "Pinu_ban", "Pinu_con"), DT) < zeroCutoff))
             ||
             any(DT$leading %in% "Lari_lar"))
  { ## Wetland :Black spruce dominated and  Deciduous species more than 20%
    "BkSpWet"
  } else if (.sumRelBs(deciSp, DT) < 0.2) {
    "ConMix"
  } else if ((any(DT$leading %in% c("Popu_tre", "Popu_bal", "Betu_pap"))) &&
